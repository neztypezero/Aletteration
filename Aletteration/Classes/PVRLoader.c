...
//  Aletteration
//
//  Created by David Nesbitt on 3/10/11.
...
TextureInfo loadPVR(const char *path) {
	TextureInfo texInfo;
	
	FILE *pvrFile = fopen(path, "rb");
	fseek (pvrFile, 0, SEEK_END);
	int fileSize = ftell(pvrFile);
	fseek (pvrFile, 0, SEEK_SET);
	
	unsigned char *fileBytes = (unsigned char *)malloc(fileSize);
	fread(fileBytes, fileSize, 1, pvrFile);

	PVR_TEXTURE_HEADER* pvrHeader = (PVR_TEXTURE_HEADER*)fileBytes;
	
	GLenum eTextureFormat = 0;
	
	char *glExtString = (char*)glGetString(GL_EXTENSIONS);
	int bIsPVRTCSupported = (strstr(glExtString, "GL_IMG_texture_compression_pvrtc") != NULL);

	int bIsCompressedFormatSupported = 0, bIsCompressedFormat = 0;
	
	unsigned int ePixelType = pvrHeader->dwpfFlags & PVRTEX_PIXELTYPE;
	switch(ePixelType) {
		case MGLPT_PVRTC2:
		case OGL_PVRTC2:
			if(bIsPVRTCSupported) {
				bIsCompressedFormatSupported = bIsCompressedFormat = 1;
				eTextureFormat = pvrHeader->dwAlphaBitMask==0 ? GL_COMPRESSED_RGB_PVRTC_2BPPV1_IMG : GL_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG ;	// PVRTC2
			}
			break;
		...
		case OGL_PVRTC4:
			if(bIsPVRTCSupported) {
				bIsCompressedFormatSupported = bIsCompressedFormat = 1;
				eTextureFormat = pvrHeader->dwAlphaBitMask==0 ? GL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG : GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG ;	// PVRTC4
			}
			break;
	}
	if(bIsCompressedFormatSupported == 1) {
		int nTextureLevelsNeeded = (pvrHeader->dwpfFlags & PVRTEX_MIPMAP)? pvrHeader->dwMipMapCount : 0;

		glGenTextures(1, &texInfo.name);
		glBindTexture(GL_TEXTURE_2D, texInfo.name);
		texInfo.level = nTextureLevelsNeeded;
		texInfo.width = pvrHeader->dwWidth;
		texInfo.height = pvrHeader->dwHeight;
		if (texInfo.level == 0) {
		...
		char *theTexturePtr = ((char*)pvrHeader + pvrHeader->dwHeaderSize);
		char *theTextureToLoad = 0;
		int		nMIPMapLevel;
		unsigned int		nSizeX= pvrHeader->dwWidth, nSizeY = pvrHeader->dwHeight;
		...
		for(nMIPMapLevel = 0; nMIPMapLevel <= nTextureLevelsNeeded; nSizeX=PVRT_MAX(nSizeX/2, (unsigned int)1), nSizeY=PVRT_MAX(nSizeY/2, (unsigned int)1), nMIPMapLevel++) {
			...
			if ((pvrHeader->dwpfFlags & PVRTEX_PIXELTYPE)==OGL_PVRTC2) {
				CompressedImageSize = ( PVRT_MAX(nSizeX, PVRTC2_MIN_TEXWIDTH) * PVRT_MAX(nSizeY, PVRTC2_MIN_TEXHEIGHT) * pvrHeader->dwBitCount) / 8;
			} else if ((pvrHeader->dwpfFlags & PVRTEX_PIXELTYPE)==OGL_PVRTC4) {
				CompressedImageSize = ( PVRT_MAX(nSizeX, PVRTC4_MIN_TEXWIDTH) * PVRT_MAX(nSizeY, PVRTC4_MIN_TEXHEIGHT) * pvrHeader->dwBitCount) / 8;
			}
